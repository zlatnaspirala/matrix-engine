			...
			if(world.contentList[physicsLooper].physics.enabled) {
				...
				if(local.physics.currentBody.shapeOrientations.length == 1) {
					...
					if(false) {
						...
						world.contentList[physicsLooper].rotation.rotx = radToDeg(local.physics.currentBody.quaternion.toAxisAngle()[1]) - 90;
						world.contentList[physicsLooper].rotation.roty = radToDeg(local.physics.currentBody.quaternion.toAxisAngle()[1]) - 0;
						world.contentList[physicsLooper].rotation.rotz = radToDeg(local.physics.currentBody.quaternion.toAxisAngle()[1]) + 0;
					} else {
