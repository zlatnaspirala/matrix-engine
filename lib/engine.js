...
import { frames,
         modifyFrames,
         objListToDispose } from "./matrix-world";
...
export function load_shaders (href) {
...
export function updateFPS(elements) {
  ...
  if (1000 < updateTime) {
    document.getElementById("fps").innerHTML = "FPS AVG:" + Math.floor((1000 * frames / totalTime) / elements)
            + " CUR:" + Math.floor((1000 * updateFrames / updateTime) / elements + " matrixEngine");
    updateTime = 0;
    updateFrames = 0;
  }
}
...
export function CANVAS2d_SURFACE_TEXTURE(path_, path_to_run_script) {
