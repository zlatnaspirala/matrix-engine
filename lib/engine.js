...
import {E, randomIntFromTo, scriptManager, QueryString} from './utility';
import {WebGLUtils} from './webgl-utils';
import {reDrawID} from './matrix-render';
import {CS3, frames, modifyFrames, objListToDispose} from './matrix-world';
...
import {BIGLOG} from '../networking2/matrix-stream';

if(App.offScreenCanvas == true || (QueryString.offScreen == 'true')) {
...
export function SET_STREAM(video) {
	...
	navigator.mediaDevices.enumerateDevices()
		.then(getDevices).then(getStream).catch(() => {
			alert('ERR MEDIA')
		});
	...
	function getStream() {
		if(navigator.getUserMedia) {
			var VIDEO__;
			if(isMobile() == true) {
				VIDEO__ = {deviceId: {exact: videoSrc}, facingMode: 'user'}
			} else {
				VIDEO__ = true;
			}
