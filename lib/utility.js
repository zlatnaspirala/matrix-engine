...
export var scriptManager = {
  ...
  loadModule: function addScript(src) {
    ...
    var s = document.createElement("script");
    s.onload = function () { scriptManager.SCRIPT_ID++; };
...
App.audioSystem.createVideoAsset = function (name_, path_) {
  ...
  // videoAudioAsset.filter.type = 2; // Change Filter type to test // ENUM from UTILITY
...
export let ENUMERATORS = {
  ...
  glDrawElements: {
    help: function () {
      SYS.DEBUG.WARNING(
        "C specification: void glDrawElements(  GLenum mode , GLsizei count , GLenum type , const GLvoid * indices ); "
      );
      SYS.DEBUG.WARNING(
        ">>>mode can be : 'POINTS' , 'LINE_STRIP', 'LINE_LOOP', 'LINES', 'TRIANGLE_STRIP', 'TRIANGLE_FAN' , 'TRIANGLES' "
      );
      SYS.DEBUG.WARNING(
  ...
  getTexParameter: function () {
    SYS.DEBUG.LOG(
      " TEXTURE_IMMUTABLE_FORMAT VALUE : " +
        world.GL.gl.getTexParameter(
          world.GL.gl.TEXTURE_2D,
          world.GL.gl.TEXTURE_IMMUTABLE_FORMAT
        )
    );
  },
};
