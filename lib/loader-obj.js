 ...
 export class constructMesh {
  ...
  constructor (objectData, inputArg) {
    ...
    this.setScale = (s) => {
      ...
      OBJ.initMeshBuffers( world.GL.gl, this.create(this.objectData, inputArg));
    };
  }
      ...
      if (VERTEX_RE.test(line)) {
        ...
        for (var j = 0, eleLen = elements.length; j < eleLen; j++) {
          ...
          if (elements[j] in unpacked.hashindices) {
            ...
            unpacked.verts.push(+verts[(vertex[0] - 1) * 3 + 2] * inputArg.scale);
...
OBJ.downloadMeshes = function (nameAndURLs, completionCallback, inputArg) {
  ...
  if (inputArg === undefined) inputArg = { scale: 2 };
  var meshes = {};
