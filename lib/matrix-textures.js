...
App.clearTextureCache = (gl, src) => {
	const tex = App.textureCache[src];
	if(tex) {
		gl.deleteTexture(tex);
		delete App.textureCache[src];
	}
}

App.tools.loadTextureImage = function(gl, src, params) {
	// Return cached texture if already loaded
	if(App.textureCache[src]) {
		return App.textureCache[src];
	}
