...
*/
import {sys, ActivateModifiers, APPLICATION, loadEditor, runEditor, loadEditorObjects, VIEW} from 'visual-js';
ActivateModifiers();

window.onresize = function(e) {
  if(sys.DOM.RESIZE_TYPE == 'DIAMETRIC') {
    sys.DOM.c.width = sys.MATH.CONVERT.PER_TO_PIX(sys.DOM.W_PIX);
    sys.DOM.c.height = sys.MATH.CONVERT.PER_TO_PIY(sys.DOM.H_PIX);
  }
  console.log("GLOBAL EVENT : ONRESIZE ")
};
// ONLY FOR DEV
window.sys = sys;
var OSCILLATOR = sys.MATH.OSCILLATOR;
    ...
    SPIN_STOP = function(win) {
      ...
      window['symbol1' + RESULTS.R1].ON_UPDATE = function() {
        if(SLOT_MASHINE.SPIN == true) {
          ...
          if(
            window['symbol1' + RESULTS.R1].POSITION.y > distance - REELS_OPTIONS.REELS_MARGIN &&
            window['symbol1' + RESULTS.R1].POSITION.y < distance + REELS_OPTIONS.REELS_MARGIN &&
            REELS_OPTIONS.REEL1_STOPED == false
          ) {
            ...
            if(RESULTS.R1 < SYMBOLS_REEL1.length - 1) {
              window['Csymbol10'].POSITION.SET_POSITION(
                REELS_OPTIONS.REEL1_POSITION_X,
                symbol10.POSITION.y - (REELS_OPTIONS.SYMBOL_HEIGHT + REELS_OPTIONS.REELS_MARGIN) * SYMBOLS_REEL1.length,
                'DIAMETRIC'
              );
            }
      ...
      window['symbol2' + RESULTS.R2].ON_UPDATE = function() {
        if(SLOT_MASHINE.SPIN == true) {
          ...
          if(
            window['symbol2' + RESULTS.R2].POSITION.y > distance - REELS_OPTIONS.REELS_MARGIN &&
            window['symbol2' + RESULTS.R2].POSITION.y < distance + REELS_OPTIONS.REELS_MARGIN &&
            REELS_OPTIONS.REEL1_STOPED == true
          ) {
            // audio_object_stop.play();
            ...
            if(RESULTS.R2 < SYMBOLS_REEL2.length - 1) {
              window['Csymbol20'].POSITION.SET_POSITION(
                REELS_OPTIONS.REEL2_POSITION_X,
                symbol20.POSITION.y - (REELS_OPTIONS.SYMBOL_HEIGHT + REELS_OPTIONS.REELS_MARGIN) * SYMBOLS_REEL2.length,
                'DIAMETRIC'
              );
            }
      ...
      window['symbol3' + RESULTS.R3].ON_UPDATE = function() {
        if(SLOT_MASHINE.SPIN == true) {
          ...
          if(
            window['symbol3' + RESULTS.R3].POSITION.y > distance - 2 * REELS_OPTIONS.REELS_MARGIN &&
            window['symbol3' + RESULTS.R3].POSITION.y < distance + 2 * REELS_OPTIONS.REELS_MARGIN &&
            REELS_OPTIONS.REEL2_STOPED == true
          ) {
            // audio_object_stop.play();
            ...
            if(RESULTS.R3 < SYMBOLS_REEL3.length - 1) {
              window['Csymbol30'].POSITION.SET_POSITION(
                REELS_OPTIONS.REEL3_POSITION_X,
                symbol30.POSITION.y - (REELS_OPTIONS.SYMBOL_HEIGHT + REELS_OPTIONS.REELS_MARGIN) * SYMBOLS_REEL3.length,
                'DIAMETRIC'
              );
            }
